...
export async function GET(request: Request, { params }: { params: Promise<{ draftId: string; path: string[] }> }) {
  try {
    ...
    if (!corsHeaders) throw new Error("unauthorized origin");
    ...
    for (const candidate of candidates(requested)) for (const filePath of [`${context.artifactDirectory}/${candidate}`, `${context.artifactDirectory}/public/${candidate}`]) {
      try {
        ...
        if (!isModule && !css) { const bytes = await context.reader.readBinary(context.source, filePath); return new Response(new Uint8Array(bytes), { headers: { "Content-Type": getPreviewAssetContentType(filePath), "Cache-Control": "no-store", "Referrer-Policy": "no-referrer", "X-Content-Type-Options": "nosniff", ...corsHeaders } }); }
        ...
        const body = isModule
          ? transformPreviewModule(text, filePath, previewQuery, { assetBaseUrl, credentialed: true, dependencies: manifest?.dependencies ?? {} })
        ...
        return new Response(body, { headers: { "Content-Type": getPreviewAssetContentType(isModule ? "module.js" : filePath), "Cache-Control": "no-store", "Referrer-Policy": "no-referrer", "X-Content-Type-Options": "nosniff", ...corsHeaders } });
